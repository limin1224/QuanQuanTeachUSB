...
//bMaxPacketSize0字段。AT90USB162的端点0大小为8字节。
 0x08,
...
//idProduct字段。产品ID号，由于是第二个实验，我们这里取0x2202。
//注意小端模式，低字节应该在前。
 0x02,
 0x22,
...
const uint8 ConfigurationDescriptor[9+9+5+5+4+5+7+9+7+7]=
{
 ...
 //bEndpointAddress字段。端点的地址。我们使用AT90USB162的输入端点3。
 //D7位表示数据方向，输入端点D7为1。所以输入端点1的地址为0x83。
 0x83,
 ...
 //bEndpointAddress字段。端点的地址。我们使用AT90USB162的输出端点4。
 //D7位表示数据方向，输出端点D7为0。所以输出端点2的地址为0x04。
 0x04,
...
//8位小端格式
const unsigned char ProductStringDescriptor[38]={
38,         //该描述符的长度为38字节
0x03,       //字符串描述符的类型编码为0x03
0x0a, 0x30, //《
0x08, 0x57, //圈
...
入口参数：无。
返    回：无。
备    注：由pSendData的最高8位决定指针的类型，当最高8位为0x01时，指向RAM区，
          否则指向FLASH区。
********************************************************************/ 
void UsbEp0SendData(void)
{
 ...
 {
  //按最大包长度发送
  if((pSendData&0xFF000000)==0x01000000)  //RAM中的数据
  {
 ...
 else
 {
  ...
  else //如果要发送的数据包长度为0
  {
   ...
   {
    UsbChipWriteEndpointBuffer(0,0,0); //发送0长度数据包
...
void UsbEp0DataOut(void)
{
 ...
 if((bmRequestType==0x21)&&(bRequest==SET_LINE_CODING))
 {
  ...
  UEINTX=~(1<<2); //清除RXOUTI，将清除中断标志以及缓冲区
  ...
  //返回0长度的状态数据包。
  UsbChipWriteEndpointBuffer(0,0,0);
 }
    ...
    case 1:  //类请求
     ...
     switch(bRequest)
     {
      case GET_LINE_CODING: //GET_LINE_CODING请求
       ...
       pSendData=0x01000000+(uint32)LineCoding;  //设置为RAM数据
      break;
...
void UsbEp4Out(void)
{
 ...
 if(UsbEp4ByteCount!=0) return;
 //清除端点4输出的中断标志位
 UEINTX=~(1<<2);
 //读端点4缓冲区数据
 UsbEp4ByteCount=UsbChipReadEndpointBuffer(4,64,UsbEp4Buffer);
 //清除端点缓冲区
 UsbChipClearBuffer(4);
 //输出位置设为0
 UsbEp4BufferOutputPoint=0; 
}
////////////////////////End of function//////////////////////////////