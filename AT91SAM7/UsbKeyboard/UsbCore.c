...
//idProduct字段。产品ID号，由于是第二个实验，我们这里取0x2102。
//注意小端模式，低字节应该在前。
 0x02,
 0x21,
...
//字符串“21IC DIY U盘学习板 之USB键盘”的Unicode编码
//8位小端格式
...
void UsbEp2Out(void)
{
 ...
 //从端点2输出缓冲读回1字节数据
 UsbChipReadEndpointBuffer(2,1,Buf);
 //清除端点缓冲区
 UsbChipClearBuffer(2);
 
 //输出报告1字节为LED状态，某位为1时，表示LED亮。
 if(Buf[0]&0x01) //Num Lock Led
 {
  OnLed2();
 }
 else
