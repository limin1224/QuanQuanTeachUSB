...
{
 //判断该返回什么数据
 if(ByteAddr==0) pEp1SendData=(uint8 *)Dbr; //返回DBR
 if(ByteAddr==512) pEp1SendData=(uint8 *)Fat; //返回FAT
 if((ByteAddr>=576)&&(ByteAddr<16896)) pEp1SendData=(uint8 *)Zeros;
 if(ByteAddr==16896) pEp1SendData=(uint8 *)Fat; //返回FAT（备份FAT）
 if((ByteAddr>=16960)&&(ByteAddr<33280)) pEp1SendData=(uint8 *)Zeros;
 if(ByteAddr==33280) pEp1SendData=(uint8 *)RootDir; //返回根目录
 if((ByteAddr>=33344)&&(ByteAddr<49664)) pEp1SendData=(uint8 *)Zeros;
 if(ByteAddr==49664) pEp1SendData=(uint8 *)TestFileData; //返回文件数据
