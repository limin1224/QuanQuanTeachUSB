...
const uint8 DiskInf[36]=
{
 ...
 //产品标识，为字符串“自己做的真U盘”
 0xD7,0xD4,0xBC,0xBA,0xD7,0xF6,0xB5,0xC4,0xD5,0xE6,0x55,0xC5,0xCC,0x00,0x00,0x00,
...
const uint8 MaximumCapacity[12]=
{
 ...
 //每块大小为512字节
 (FLASH_SECTOR_SIZE>>16),(FLASH_SECTOR_SIZE>>8),FLASH_SECTOR_SIZE
};
...
{
 //能够访问的最大逻辑块地址
 (FLASH_MAX_SECTOR_ADDR/FLASH_SECTOR_SIZE)>>24,
 (FLASH_MAX_SECTOR_ADDR/FLASH_SECTOR_SIZE)>>16,
 (FLASH_MAX_SECTOR_ADDR/FLASH_SECTOR_SIZE)>>8,
 FLASH_MAX_SECTOR_ADDR/FLASH_SECTOR_SIZE,
 //块的大小
 (FLASH_SECTOR_SIZE>>24), (FLASH_SECTOR_SIZE>>16),
 (FLASH_SECTOR_SIZE>>8), FLASH_SECTOR_SIZE
};
...
{
 if(0==(ByteAddr&(FLASH_SECTOR_SIZE-1)))  //如果是扇区的整数倍
 {
  //从FLASH中读出一个扇区
  FlashReadOneSector(ByteAddr,FlashSectorBuf,Ep1DataLength/FLASH_SECTOR_SIZE-1);
  pEp1SendData=FlashSectorBuf;  //指向缓冲区首地址
 }
...
void ProcScsiCommand(void)
{
 ...
 switch(CBW[15])
 {
  ...
  #endif
   Ep1DataLength=GetDataTransferLength(); //获取需要返回的长度
   ByteAddr=GetLba()*FLASH_SECTOR_SIZE; //获取字节地址，字节地址为逻辑块地址乘以每块大小
...
void ProcScsiOutData(void)
{
 ...
 ByteAddr+=Len; //调整地址
 if(0==(ByteAddr&(FLASH_SECTOR_SIZE-1))) //如果地址是扇区整数倍，则需要写数据到FLASH
 {
  //写一个扇区数据。注意地址要调整到扇区的开始处
  FlashWriteOneSector(ByteAddr-FLASH_SECTOR_SIZE,FlashSectorBuf,Ep2DataLength); 
