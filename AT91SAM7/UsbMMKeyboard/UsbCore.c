...
//idProduct字段。产品ID号，由于是第三个实验，我们这里取0x2103。
//注意小端模式，低字节应该在前。
 0x03,
 0x21,
...
const uint8 ConfigurationDescriptor[9+9+9+7+7]=
{
 ...
 //bEndpointAddress字段。端点的地址。这里使用AT91SAM7的输出端点2。
 //D7位表示数据方向，输出端点D7为0。所以输出端点1的地址为0x02。
 0x02,
...
//字符串“21IC DIY U盘学习板 之USB多媒体键盘”的Unicode编码
//8位小端格式
const uint8 ProductStringDescriptor[50]={
 50,         //该描述符的长度为50字节
 0x03,       //字符串描述符的类型编码为0x03
 0x32, 0x00, //2
 0x31, 0x00, //1
...
void UsbEp2Out(void)
{
 ...
 if(Buf[0]!=0x01) return;  //如果报告ID不是1，则说明不是LED控制报告
 //输出报告第2字节为LED状态，某位为1时，表示LED亮。
 if(Buf[1]&0x01) //Num Lock Led
 {
  OnLed2();
 }
 else
