...
void UsbChipSetEndpointEnable(uint8 Enable)
{
 if(Enable!=0)
 {
  AT91C_UDP_CSR[1]|=(7<<8)|(1<<15); //使能端点1，并设置为中断IN端点
  while((AT91C_UDP_CSR[1]&((7<<8)|(1<<15)))!=((7<<8)|(1<<15)));  //等待设置好
 
  AT91C_UDP_CSR[2]|=(3<<8)|(1<<15); //使能端点2，并设置为中断OUT端点
  while((AT91C_UDP_CSR[2]&((3<<8)|(1<<15)))!=((3<<8)|(1<<15)));  //等待设置好
 }
 else
 {
 }
...
void UsbChipClearBuffer(uint8 Endp)
{
 ...
 {
  AT91C_UDP_CSR[Endp]&=~(1<<1); //将RX_DATA_BK0置0
  while(AT91C_UDP_CSR[Endp]&(1<<1));  //等待该位被置0
 }
 else
 {
  AT91C_UDP_CSR[Endp]&=~(1<<6); //将RX_DATA_BK1置0
  while(AT91C_UDP_CSR[Endp]&(1<<6));  //等待该位被置0
 }
}
////////////////////////End of function//////////////////////////////
...
{
 AT91C_UDP_CSR[Endp]|=(1<<7);  //设置DIR位
 while(!(AT91C_UDP_CSR[Endp]&(1<<7)));  //直到该位被设置
 AT91C_UDP_CSR[Endp]&=~(1<<2);  //清除RX SETUP位
 while((AT91C_UDP_CSR[Endp]&(1<<2)));  //直到该位被清除
}
////////////////////////End of function//////////////////////////////
...
{
 AT91C_UDP_CSR[Endp]|=(1<<4); //TXPKTRDY置1，使能发送数据
 while(!(AT91C_UDP_CSR[Endp]&(1<<4))); //等待该位被置1
}
////////////////////////End of function//////////////////////////////
